 ...
 * @project 	: 1
 * @fileName	: parser.c
 * @purpose     : To generate a parse tree by taking tokens from the lexan(), for checking the syntax of the given input file
 ************************************/
    ...
    while(1) {
        switch(lookahead) {
            case '*': case '/': case DIV: case MOD:
                t = lookahead;
                match(lookahead); factor();oper(t);//emit(t, NONE);
		continue;
            default:
		    return;
        }
